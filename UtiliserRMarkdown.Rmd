...
Ce document a été conçu comme une introduction à l'utilisation de R Markdown, il permet

  1. d'apprendre la syntaxe Pandoc Markdown;
  2. de se familiariser les fonctionnalités du packages `rmarkdown`;
  3. de découvrir un certains nombre de resources disponibles en ligne pour aller plus loin.

Ce document est lui-même écrit avec R Markdown et peut-être convertit en une
page web (au format `.html`), en forma (.`.docx`) ou en PDF. Comme vous l'aurez vraisemblablement noté, dans "R Markdown", il y a "R" et "Markdown" et dans ce document je ne fais pas d'intriduction a R et j'invite les utilisateurs non-familier avec ce dernier de se reporter à l'une des différentes introduction disponible sur le site du CRAN: <https://cran.r-project.org/other-docs.html>.
...
compilation, la version `r packageVersion("rmarkdown")` du package `rmarkdown` a
été utilisée. De plus amples informations relatives à l'environnement dans
...
En milieu universitaire, de nos travaux préliminaires à nos manuscripts, nous
produisons très souvent des documents intégrant analyses et figures. Il est
alors convenient de pouvoir les intégrer directement dans un document complet
qui peut également, au besoin, inclure le code source associé. Avec
...
Nous devons la syntaxe "Markdown original" à John Gruber, avec une la version
1.0.1 disponible sur son site internet depuis décembre 2004
