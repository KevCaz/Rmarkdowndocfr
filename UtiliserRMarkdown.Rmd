...
Dans `R Markdown`, il y a R et Markdown. Pour la suite du document, le lecteur doit être est familier avec [R](http://www.r-project.org)[^niveau]. Il me reste alors à parler de Markdown. Le ["Mardown original"](http://daringfireball.net/projects/markdown/)[^niveau] date de 2004, l'idée de départ est simple et élégante&nbsp;: produire un langage léger qui simplifie les balises HTML et facilement transformé en HyperText Markup Language (HTML, utilisé par tout les sites Internet). L'idée n'est pas tant de remplacer le HTML mais plutôt d'en augmenter l'efficacité d'écriture. Chemin faisant, Markdown séduit, en parallèle de la version originale (qui n'a jamais été standardisée), plusieurs extensions voient le jour. Il s'agit surtout de lever certaines limitations tout en préservant l'esprit d'origine, parmi elles:
...
2. [un cook d'atuces](https://bookdown.org/yihui/rmarkdown-cookbook/)  en cours d'écriture.
...
[^niveau]: Le lecteur doit être capable à minima d'utiliser des fonctions et de charger des packages.
...
L'argument "all" permet d’obtenir l'ensemble des fichiers pour lequel il existe une spécification dans le fichier YAML. Les formats utilisés ici sont&nbsp;:
...
En utilisant une indentation de 4 espaces (ou une tabulation), on peut obtenitr
...
## Une  application, modèle linéraire dynamique
...
# De nombreuse utilisation de R Markdown

WIP !! WIP !! WIP !!

Nous concluons ce document par une ouverture sur davantage de fonctionalité du package R Markdown.
