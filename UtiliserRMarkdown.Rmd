...
Ce document a été conçu comme une introduction à R Markdown avec les objectifs suivants:
...
page internet (au format `.html`), en format Word (.`.docx`) ou en PDF. Bien
que dans "R Markdown", il y ait "R" et "Markdown", ce document n'est pas une
introduction à R. J'invite les utilisateurs non-familiers R à se reporter à
l'une des différentes introduction disponibles, par exemple, sur le site du
...
Dans de nombreux milieux professionnels, à des fins de communication diverses et
variées, sont produits régulièrement des documents intégrant analyses et
figures. Pour créer de tels documents, il faut être en mesure de faire les
analyses analyses et figures pour les intégrer dans le document final.
[R](http://www.r-project.org) est un langage de programmation dédié à l'analyse
de données qui, au travers de nombreux packages, permet de manipuler et traiter
un spectre très large de données et de les visualiser efficacement. Il est aussi
possible d'intégrer de les intégrer directement dans un

Il est alors
convenient de pouvoir les intégrer directement dans un document complet qui peut
également, au besoin, inclure le code source associé. Avec
[R](http://www.r-project.org), c'est possible depuis de nombreuses années avec
package de base `utils`, qui inclut la function `Sweave()` qui permet de
remplacer du code R par son rendu. Le package
[`knitr`](http://cran.r-project.org/web/packages/knitr/index.html), qui utilise
cette fonctionnalité, permet de produire des fichiers LaTeX intégrant du code R
et des figures. Le package [`rmarkdown`](https://rmarkdown.rstudio.com/)
développé par RStudio est un package R très populaire qui a rendu la création de
documents dynamiques avec R très efficace.

Nous devons la syntaxe originale de Markdown à John Gruber, programmer et
blogger et baladodiffuseur de Philadelphie. Sur son site "daringfireball", Markdown est décrit sur son site
depuis décembre 2004 et on peut même y téléchager (voir
<http://daringfireball.net/projects/markdown>. L'idée de départ est simple et
élégante&nbsp;: produire un langage léger qui simplifie les balises HTML et
facilement transformé en HyperText Markup Language (HTML, utilisé par tout les
sites Internet). L'idée n'est pas tant de remplacer le HTML mais plutôt d'en
augmenter l'efficacité d'écriture. Chemin faisant, Markdown séduit, en parallèle
de la version originale (qui n'a jamais été standardisée), plusieurs extensions
voient le jour. Il s'agit surtout de lever certaines limitations tout en
préservant l'esprit d'origine, parmi elles:
...
À noter aussi l'initiative [CommonMark](https://commonmark.org/) qui est une
spécification (norme technique) pour Markdown de plus en plus utilisées. R
Markdown(http://rmarkdown.rstudio.com)[^1] n'est pas une nouvelle variante
Markdown. Il s'agit de d'utiliser de la variante [Pandoc
Markdown](https://rmarkdown.rstudio.com/lesson-8.html)[^2] et d'y ajouter les
fonctionnalités d'autres packages, notamment
[`knitr`](http://cran.r-project.org/web/packages/knitr/index.html) pour intégrer
du code R et ce qu'il produit dans le document.  et
[`yaml`](http://cran.r-project.org/web/packages/yaml/index.html) e tout lié par
l'utilisation de [Pandoc](http://www.pandoc.org/installing.html). Une précision
relative à [Pandoc](http://www.pandoc.org/installing.HTML) s'impose. Il s'agit,
comme l'indique le site internet, d'un "convertisseur de document universel", en
une ligne de commande Pandoc convertit un document dans un format donné en de
nombreux autres formats. Par exemple, on peut passer d'un fichier `.tex` (LaTeX)
à un fichier `.docx` (Word)! Cela étant dit, revenons-en à `rmarkdown`. Les
intérêts de ce package `sont nombreux :
...
Concernant la documentation relative aux fonctionnalités du package `rmarkdown`, je recommande [le site officiel de R Markdown](http://rmarkdown.rstudio.com) sur lequel vous trouverez entre autres

1. un condensé d'utilisation sous forme de [*Cheat Sheet*](http://www.rstudio.com/wp-content/uploads/2015/02/rmarkdown-cheatsheet.pdf);
2. [guide de référence](http://www.rstudio.com/wp-content/uploads/2015/03/rmarkdown-reference.pdf).

Il y a également sur le site <https://bookdown.org>

1. un livre en anglais
très complet sur le sujet [disponible en
ligne](https://bookdown.org/yihui/rmarkdown/) ;
2. [un cook d'astuces](https://bookdown.org/yihui/rmarkdown-cookbook/)  en cours d'écriture.


 Le [site dédié à knitr](http://yihui.name/knitr/) permet de bien comprendre l'intégration des morceaux de codes et des figures de [R](http://www.r-project.org). Je tiens également à signaler la [présentation de Mansun Kuo sur Rpubs](http://rpubs.com/mansun_kuo/24330). Pour apprendre la syntaxe Pandoc Markdown, vous pouvez vous reportez à la [documentation disponible sur le site de Pandoc](http://pandoc.org/demo/example9/pandocs-markdown.html) ou à la [page internet de Jean-Daniel Bonjour](http://enacit1.epfl.ch/markdown-pandoc/#notes_bas_page) pour une documentation en français.
...
la mise en page de votre document. Néanmoins, avant d'envisager l'un des recours
proposés ci-dessous, il est très important de vous demander si cette
personnalisation est nécessaire. Si toutefois vous souhaitez aller plus loin
dans la mise en forme de votre document, plusieurs options vous sont
offertes&nbsp;:
...
[^3]: Vous les trouverez dans le dossier d

u package `rmarkdown`. Pour savoir où est ce dossier, enter "R.home()" dans votre console R, rendez-vous à l'adresse donnée puis, dans le sous-répertoire "library/rmarkdown".
...
Un fichier R Markdown (`.rmd` ou `.Rmd`), c'est :

1. un document de texte plein qui utilise la syntaxe Pandoc Markdown:
- des blocs de code particuliers (bloc de trois backticks \` avec une accolade \{R ...\}) qui peuvent exécuté par R:

        ````{R nom, option1, option2}
        # code R à exécuter
        ```
- enfin, un Front Matter YAML optionnel (au début du fichier, dans un bloc de trois tirets&nbsp;: \-\-\-) qui permet de donner des indications sur le ou les documentes à générer.
...
Concluons ce document par une ouverture sur davantage de fonctionalité du package R Markdown.
