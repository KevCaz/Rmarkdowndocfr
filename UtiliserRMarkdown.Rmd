...
documents dynamiques avec R très efficace. Notez qu'il y a un livre en anglais
très complet sur le sujet [disponible en
ligne](https://bookdown.org/yihui/rmarkdown/) et [un cook d'atuces](https://bookdown.org/yihui/rmarkdown-cookbook/) est en cours d'écriture.
...
À noter aussi l'initiative [CommonMark](https://commonmark.org/) qui est une spécification (norme technique) pour Markdown de plus en plus utilisées. R Markdown(http://rmarkdown.rstudio.com)[^1] n'est pas une nouvelle variante Markdown. Il s'agit de d'utiliser de la variante [Pandoc Markdown](https://rmarkdown.rstudio.com/lesson-8.html)[^2] et d'y ajouter les fonctionnalités d'autres packages, notamment [`knitr`](http://cran.r-project.org/web/packages/knitr/index.html) pour intégrer du code R et ce qu'il produit dans le document.  et [`yaml`](http://cran.r-project.org/web/packages/yaml/index.html) e tout lié par l'utilisation de [Pandoc](http://www.pandoc.org/installing.html). Une précision relative à [Pandoc](http://www.pandoc.org/installing.HTML) s'impose. Il s'agit, comme l'indique le site internet, d'un "convertisseur de document universel", en une ligne de commande Pandoc convertit un document dans un format donné en de nombreux autres formats. Par exemple, on peut passer d'un fichier `.tex` (LaTeX) à un fichier `.docx` (Word)! Cela étant dit, revenons-en à ``rmarkdown`. Les intérêts du package `rmarkdown` sont nombreux :
...
Un fichier Rmarkdown (`.rmd` ou `.Rmd`) est divisé en trois&nbsp;:
...
<!-- # De nombreuse utilisation de RMarkdown -->

<!-- Je voudrais finir par un tour d'horizon -->
