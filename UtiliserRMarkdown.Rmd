...
Un champ données peut contenir une chaine de charactère, une date, des chiffres ou encore une liste, pa
...
Noté l'indentation utilisée, c'est capital dans  dans YAML car c'est
Dans YAML, l'indentation est très importante.

et les sous-champ peuvent eux aussi être nommées
...
Quelques remarques importantes "y, yes, true, TRUE" et "n, no, false, FALSE"
seront interprétées  comme variables booléennes et donc `TRUE`, c'est une
fonctionnalité bien pratique mais a double tranchant: si j'utilise `y`, le champ
sera lui-même indiqué comme `TRUE` et donc il faut ajouter des guillemets.

<!-- KC: J'en suis la, à relire -->
**WIP** **WIP** **WIP**

Les remarques précédentes sont générales et vous pouvez facilement utiliser YAML
pour autre chose que R Markdown. Revenons au fichier R Markdown dans lequel on
utilise YAML d'une manière plus spécifique. Premièrement, les indications sont
en début de document. Deuxièmement les champs concernent le format de sortie du
document dynamique édité avec R Markdown. Troisièmement, seuls les champs qui
sont définis dans des templates sont valides. Arrêtons-nous sur ce dernier
point. On ne peut pas utiliser n'importe quelle champ.
<!-- Autrement dit, si vous au, appelées métadonnées, au début du document entre bloc de trois tirets. -->

Pour un aperçu assez complet des options YAML utilisables dans fichier R
Markdown, rendez-vous à la dernière page du [guide de
