...
faut donc être en mesure de manipuler des données les analyser et créer des
figures pour les intégrer dans le document final. [R](http://www.r-project.org)
est un langage de programmation qui répond à ces besoins. Au travers de
nombreux packages, R permet de manipuler et traiter un spectre très large de
données et de les visualiser efficacement. Ce langage offre également la
possibilité d'intégrer ces éléments directement dans un document, notamment
avec le package très populaire `rmarkdown` qui permet d'intégrer du code R et
code et les résultats de ce code (nombre, tableaux, figures) dans un document
écrit avec Markdown et qui pourra être converti en de nombreux format de document (dont pdf, word, html).

Écrire un document en Markdown, quésaco? Markdown est un langage de balisage
léger, c'est-à-dire un langage dans lequel on peut utiliser des ensembles
caractères spécifiques (des balises) pour délimiter une zone de texte pour
laquelle un formatage spécifique est appliqué. Markdown est aujourd'hui très
répandu et il en existe différentes syntaxes. La syntaxe originale de Markdown
est le fruit du travail de John Gruber, programmer, blogger et
baladodiffuseur de Philadelphie. Sur son site "daringfireball", Markdown est
décrit sur son site depuis décembre 2004 et on peut même y télécharger la
...
les balises HTML ( utilisé par tout les sites Internet). L'idée n'est pas tant
de remplacer le HTML mais plutôt d'en augmenter l'efficacité d'écriture et de
fait, il est beaucoup plus rapide d'écrire en Markdown qui couvre les
opérations de formatage les plus courantes (listes, hyperliens, etc.). 


Chemin faisant, Markdown séduit, en
parallèle de la version originale (qui n'a jamais été standardisée), plusieurs
extensions voient le jour. Il s'agit surtout de lever certaines limitations
tout en préservant l'esprit d'origine, en voici une liste :
...
Ainsi Le Markdown devient une langage qui simplifie les actions les plus courantes d'autres autres et un point d'entrer pour aller rapidement, il n'est donc pas une alternative à ces-derniers. 
...
Dans cette partie il est question de l'organisation générale d'un fichier R Markdown qui a pour extension `.Rmd` (ou `.rmd`). 
