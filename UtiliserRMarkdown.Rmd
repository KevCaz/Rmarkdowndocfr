...
L'ensemble du code R extrait de `UtiliserRMarkdown.Rmd` sera écrit dans le fichier `code.R` qui peut-être utilisé tel quel. Les blocs de code seront facilement identifiés avec des commentaires incluant le nom donné au bloc de code ainsi que les options choisies. Par exemple, ci-dessous est présenté le bloc de code de la section [Modifier l'affichage du code source avec le paramètre `echo`].
